...
object CoreConfigManager {
    ...
    private fun configureByeDpiOutbound(v2rayConfig: V2rayConfig) {
        // Configuration describes the requested transport even during offline export.
        // CoreVpnService must acquire the runtime before starting this configuration.
        if (!MmkvManager.decodeSettingsBool(AppConfig.PREF_DPI_ENABLED, false)) return
    ...
    */
    ...
    private fun configureDns(
        ...
        // The generated default is DNS-over-TCP, so it is safe to carry through
        // WS/TLS profiles that cannot transport UDP. Keeping it in the proxy also
        // avoids mobile providers silently dropping direct UDP/53 replies.
        val dnsProxyBalancerTag = policyGroupBalancerTags[AppConfig.TAG_PROXY]
        if (dnsProxyBalancerTag != null) {
            v2rayConfig.routing.rules.add(
                V2rayConfig.RoutingBean.RulesBean(
