...
Исправлены стандартные настройки DNS, мешавшие открытию сайтов на части профилей: VPN DNS — `8.8.8.8`, локальный DNS и Fake DNS выключены. Старые стандартные настройки заменяются при обновлении один раз; другие заданные DNS сохраняются.

DPI Auto теперь использует стратегии Maximum. Проверено пользователем на Android: сайты и Instagram работают на ранее проблемном профиле.

[Описание выпуска](RELEASE_NOTES.md) · [Скачать APK](https://github.com/anonymouskeys/Dragon-vpn/releases/latest)

### Предыдущие улучшения

### DragonVPN v2.3.3

**DragonVPN v2.3.3** — стабильный подписанный выпуск для Android, рассчитанный на повседневную работу с большими подписками и сложными сетями. Это самостоятельный клиент DragonVPN со своим оформлением, логикой управления профилями, инструментами проверки и встроенными средствами обхода DPI.

### Что улучшено

- **Исправлен запуск DPI** — конфигурация ByeDPI переведена с удалённого Xray-поля `proxySettings` на актуальный транспортный параметр `streamSettings.sockopt.dialerProxy`.
- **Обновление без потери данных** — официальный APK подписан постоянным ключом проекта и устанавливается поверх `v2.3.1` и `v2.3.2` с сохранением профилей и настроек.
- **Исправлен импорт** — удалён повторяющийся пункт «Импорт из файла».
- **Фирменный статус VPN** — вместо стандартной буквы `V` отображается значок DragonVPN.
- **Большие подписки без искусственного лимита** — приложение больше не ограничивает обработку и отображение списков первыми 2500 профилями.
- **Мгновенная смена тестируемой группы** — при переходе на другую вкладку предыдущая проверка останавливается, после чего можно сразу запустить тест новой группы.
- **Корректное завершение тестов** — счётчик запущенных проверок исчезает после окончания или отмены тестирования.
- **Надёжное подключение** — усилена защита от двойного запуска, зависших состояний и конфликтов при быстром отключении и повторном подключении.
- **Ручное ограничение фрагментов** — максимальное число фрагментов по умолчанию не задано; ограничение применяется только если пользователь укажет его сам.
- **Проверяемые релизы** — CI проверяет подпись APK, контрольную сумму и наличие нативных библиотек для `arm64-v8a`, `armeabi-v7a`, `x86` и `x86_64`.

### Установка и обновление

1. Скачайте `DragonVPN-v2.3.4-universal.apk` только со страницы [официальных релизов](https://github.com/anonymouskeys/Dragon-vpn/releases/latest).
2. Если у вас установлена официальная версия `v2.3.1` или `v2.3.2`, откройте APK и подтвердите обновление — удалять старое приложение не нужно.
3. Если Android сообщает о конфликте пакета, не удаляйте старую версию до резервного копирования профилей: это означает, что установленное приложение было подписано другим ключом.

> DragonVPN не содержит рекламы, телеметрии и обязательной регистрации. Конфигурации и доступность соединения зависят от используемого сервера, протокола и сети.
